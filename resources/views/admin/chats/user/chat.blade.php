...
@section('content')
<div class="container">
    <h4>Chat with Seller #{{ $chat->seller_id }}</h4>
    ...
    fetch('/messages/' + chat_id)
        ...
        .then(messages => {
            ...
            messages.forEach(msg => {
                let who = msg.sender_type == 'user' ? '🧍You' : '🏪Seller';
...
channel.bind('App\\Events\\MessageSent', function(data) {
    if (data.message.chat_id == chat_id) {
        let who = data.message.sender_type == 'user' ? '🧍You' : '🏪Seller';
...
channel.bind('App\\Events\\UserTyping', function(data) {
    if (data.chat_id == chat_id && data.sender_type != 'user') {
        $('#typing-status').text('Seller is typing...');
